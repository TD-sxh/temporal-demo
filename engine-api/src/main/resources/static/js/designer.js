...
function createDefaultNodeData(type, id) {
    ...
    switch (type) {
        case 'START':
            return { ...base, id: '__start__', name: 'Start', inputSchema: {} };
        case 'END':
...
function showProperties(dfId) {
    ...
    switch (data.type) {
        case 'START':
            html += propGroup('输入参数约束 (inputSchema)', textareaField('prop-input-schema',
                jsonPretty(data.inputSchema || {}),
                '{"paramName": {"type":"string","required":true,"description":"..."}}'  ));
            html += `<div class="prop-group"><div class="prop-hint">定义触发此流程所需的输入参数<br>required: 是否必填, defaultValue: 默认值, description: 描述</div></div>`;
            break;
...
function bindPropertyInputs(dfId) {
    ...
    switch (data.type) {
        case 'START':
            bindJson('prop-input-schema', (val) => { data.inputSchema = val; });
            break;
...
function bindToolbar() {
    ...
    document.getElementById('btn-batch').addEventListener('click', actionBatchStart);
...
function actionRun() {
    ...
    // Try to fetch input schema and auto-populate the input vars textarea
    fetch(`${API.engine}/schema/${encodeURIComponent(type)}`)
        .then(r => r.json())
        .then(res => {
            const schema = res.inputSchema;
            if (schema && Object.keys(schema).length > 0) {
                const template = {};
                for (const [key, param] of Object.entries(schema)) {
                    template[key] = param.defaultValue != null ? param.defaultValue
                        : (param.type === 'number' ? 0 : '');
                }
                document.getElementById('run-input-vars').value = JSON.stringify(template, null, 2);
                // Show schema hint
                let hint = document.getElementById('run-schema-hint');
                if (!hint) {
                    hint = document.createElement('div');
                    hint.id = 'run-schema-hint';
                    hint.className = 'schema-hint';
                    const textarea = document.getElementById('run-input-vars');
                    textarea.parentNode.insertBefore(hint, textarea.nextSibling);
                }
                hint.innerHTML = Object.entries(schema).map(([k, p]) =>
                    `<span class="${p.required ? 'required' : 'optional'}">${k}</span>: ${p.description || p.type}${p.required ? ' <b>*必填</b>' : ''}`
                ).join('<br>');
            }
            showModal('modal-run');
        })
...
function actionBatchStart() {
    const type = document.getElementById('wf-type').value.trim();
    if (!type) {
        toast('请填写 Flow Type 以批量启动', 'error');
        return;
    }
    document.getElementById('batch-type-label').textContent = type;
    document.getElementById('batch-result').classList.add('hidden');

    // Try to fetch schema to show a per-item template
    fetch(`${API.engine}/schema/${encodeURIComponent(type)}`)
        .then(r => r.json())
        .then(res => {
            const schema = res.inputSchema;
            if (schema && Object.keys(schema).length > 0) {
                ...
                const sample = [{ inputVariables: template }, { inputVariables: template }];
                document.getElementById('batch-items').value = JSON.stringify(sample, null, 2);
            }
...
function bindModals() {
    ...
    // Batch-start modal: confirm
    document.getElementById('btn-batch-confirm').addEventListener('click', () => {
        const type = document.getElementById('wf-type').value.trim();
        let items;
        try {
            items = JSON.parse(document.getElementById('batch-items').value || '[]');
        } catch (e) {
            toast('Items JSON 格式错误', 'error');
            return;
        }
        if (!Array.isArray(items) || items.length === 0) {
            toast('请提供至少一个 item', 'error');
            return;
        }
        ...
        resultEl.className = '';
        resultEl.classList.remove('hidden');
        resultEl.textContent = '批量启动中...';

        fetch(`${API.engine}/batch-start`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(res => {
            if (res.error) {
                ...
                const cls = res.failed > 0 ? (res.succeeded > 0 ? 'warning' : 'error') : 'success';
                resultEl.className = cls;
                let text = `总计: ${res.total}, 成功: ${res.succeeded}, 失败: ${res.failed}\n`;
                (res.results || []).forEach(r => {
                    text += `  [${r.index}] ${r.status} ${r.workflowId || ''}${r.error ? ' - ' + r.error : ''}\n`;
                });
...
function cleanNode(node) {
    ...
    // START/END are simple — only keep id, name, type, next (for START), and inputSchema (for START)
    if (node.type === 'START' || node.type === 'END') {
        ...
        if (node.type === 'END') delete node.inputSchema;
        // Keep inputSchema for START if non-empty
        if (node.type === 'START' && node.inputSchema && Object.keys(node.inputSchema).length === 0) {
